...
پیشرفت نصب در session مرورگر نگه‌داری می‌شود تا بارگذاری مجدد صفحه باعث از دست رفتن اطلاعات نشود. جابه‌جایی بین مراحل با AJAX انجام می‌شود و هنگام بررسی اتصال، اجرای migration و ذخیره تنظیمات، وضعیت پردازش قابل‌مشاهده است؛ در صورت غیرفعال‌بودن JavaScript فرم‌ها همچنان با بارگذاری معمولی کار می‌کنند. در هر مرحله می‌توانید با «بازگشت» مرحله قبل را اصلاح کنید. «شروع مجدد» فقط پس از تأیید کاربر، session نصب را پاک می‌کند و فرایند را از ابتدا آغاز می‌کند. اگر دیتابیس در میانه نصب بیرون از برنامه حذف یا با یک دیتابیس خالی جایگزین شود، نصب‌کننده وضعیت نامعتبر را تشخیص می‌دهد و به مرحله مناسب دیتابیس یا migration بازمی‌گردد.
